 ...
 * @description Forces hover-only animations to play automatically when an animated asset is already available (avatars, server icons, banners, emojis, nameplates, role gradients etc.). Note: Does not unlock Nitro assets.
...
module.exports = class AutoAnimate {
    ...
    createNameplateWrapper() {
        return ({ args, originalFn }) => {
    ...
    patchNameplates() {
        ...
        const NameplateWrapper = this.createNameplateWrapper();
        Patcher.instead(mod, "Nameplate", (_thisObj, args, originalFn) => {
            return React.createElement(NameplateWrapper, { args, originalFn });
        });
    }
    patchRoleGradients() {
